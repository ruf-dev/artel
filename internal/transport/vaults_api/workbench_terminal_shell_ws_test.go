...
// TestWorkbenchTerminalShell_AuthLinkCacheRoundTrips confirms setPendingTerminalAuthLink,
// PendingTerminalAuthLink and ClearPendingTerminalAuthLink round-trip correctly on a bare handler
// — no fake ttyd server / WS relay needed, since the cache itself no longer has any terminal-
// traffic-driven clearing logic to exercise (see the authLinks field's doc comment: GetVault is
// now the only thing that ever clears an entry, once a live login-status check confirms it).
func TestWorkbenchTerminalShell_AuthLinkCacheRoundTrips(t *testing.T) {
	handler := NewWorkbenchTerminalShellHandler(nil, nil, nil)

	vaultID := uuid.New()
	otherVaultID := uuid.New()

	if got := handler.PendingTerminalAuthLink(vaultID); got != "" {
		t.Fatalf("PendingTerminalAuthLink before anything cached = %q, want empty", got)
	}

	handler.setPendingTerminalAuthLink(vaultID, authLinkForWsTest)

	if got := handler.PendingTerminalAuthLink(vaultID); got != authLinkForWsTest {
		t.Fatalf("PendingTerminalAuthLink after set = %q, want %q", got, authLinkForWsTest)
	}

	// A different vault's cache entry is untouched.
	if got := handler.PendingTerminalAuthLink(otherVaultID); got != "" {
		t.Fatalf("PendingTerminalAuthLink for unrelated vault = %q, want empty", got)
	}

	handler.ClearPendingTerminalAuthLink(vaultID)

	if got := handler.PendingTerminalAuthLink(vaultID); got != "" {
		t.Fatalf("PendingTerminalAuthLink after clear = %q, want empty", got)
	}

	// Clearing an already-empty/never-set entry is a no-op, not a panic.
	handler.ClearPendingTerminalAuthLink(otherVaultID)
}
